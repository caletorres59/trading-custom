...
**Actualización 2026-09-07 — filtro de régimen en `volatility_spike` (roadmap #2):** `volatility_spike` es una estrategia de momentum; en mercado lateral/de reversión sus "spikes" se devuelven y las operaciones solo sangran comisiones (el tramo plano 2026-08-29 a 09-05). Nuevo filtro opcional (`er_period` / `er_threshold` en `config/config.yaml`): una señal de spike se suprime (`NO_TRADE` / `CHOPPY_REGIME`) salvo que el *efficiency ratio* de Kaufman sobre las últimas `er_period` velas (movimiento neto / suma de movimientos absolutos; 0 = puro chop, 1 = tendencia limpia) sea ≥ `er_threshold`. Barrido 90d BTC+ETH (`allow_short=false`, `spike_multiplier=3.0` fijo, comisión real de Alpaca 0.25%): **las 20 combinaciones probadas** (`er_period` 20-50 × `er_threshold` 0.20-0.40) superaron a la baseline solo-largo sin filtro (BTC -4.0% / ETH -0.3%) en **ambos** activos, casi todas con menos drawdown. Valores en vivo: `er_period: 30`, `er_threshold: 0.35` → BTC +7.2% / ETH +12.7% en la ventana, drawdown ~5-8%, operaciones de ~420 a ~150. Sigue siendo un solo backtest histórico, no una ventaja validada (Regla 4). Apagado por defecto en el código (`er_period=0`); solo el `config.yaml` lo activa.

**Actualización 2026-09-18 — revisión en vivo, ETH-USD agregado, salidas asimétricas probadas y rechazadas (roadmap #3):** un chequeo del estado en vivo (11.5 días desde la migración a Alpaca) encontró la cuenta **completamente congelada 10 días seguidos** (09-07 a 09-17, $99,938.12 sin moverse un centavo) — el filtro de régimen hizo lo que se le pidió (no operar en un tramo lateral/bajista de BTC, $80,339→$76,145) pero eso dejó la tasa de disparo en 0.13% (4 señales no-`NO_TRADE` de 3,176). Solo 2 operaciones ejecutadas en 11.5 días, contra las ~20 esperadas por el backtest de la idea #2 (~1.7/día) — un déficit de 10x. Dos cambios evaluados en paralelo:
- **`ETH-USD` agregado a `symbols` (SHIPPED).** Barrido 90d fresco (comisión real 0.25%, parámetros en vivo actuales, cuenta compartida $100k / 50% exposición — el mismo montaje que corre en vivo, no dos cuentas separadas): BTC solo +10.45% (maxDD 4.74%, Sharpe 2.53, 124 operaciones) vs. canasta BTC+ETH +10.75% (maxDD 6.32%, Sharpe 1.87, **251 operaciones — ~2x la frecuencia**). El retorno total mejora poco y el perfil ajustado a riesgo (Sharpe, drawdown) es algo peor que BTC solo — BTC y ETH están muy correlacionados, poca diversificación real (misma lección que el rechazo de la idea #1). Se envía igual por el beneficio de frecuencia de operación, que es justo lo que este chequeo encontró roto.
- **Salidas asimétricas / trailing-stop (roadmap #3) — CONSTRUIDO pero NO activado.** Nuevo `src/aurora/risk/trailing_stop.py` (`TrailingStopEngine`, determinista, corre cada tick independientemente de la señal de la estrategia — mismo patrón que el kill-switch), integrado en ambos motores de backtest y en el loop en vivo (con persistencia en `position_stop_state` para sobrevivir entre corridas). Barrido de grilla 90d BTC+ETH (`stop_loss_pct` 2-12% × `trail_activation_pct` 2-20% × `trail_pct` 1-10%, comisión real 0.25%): **ninguna combinación mejoró la baseline sin salidas** — los umbrales sueltos nunca dispararon (idénticos a la baseline) y los umbrales más ajustados empeoraron ambos activos. Aislando las dos piezas: el stop-loss duro casi nunca dispara con el tamaño de posición actual (10% máximo por operación no deja que una sola posición caiga tan lejos antes de que la propia estrategia o el kill-switch ya hayan actuado), y el trailing-stop interrumpe la forma real en que esta estrategia captura una tendencia — reentradas escalonadas en spikes sucesivos, no una sola posición sostenida — forzando una venta y recompra innecesaria (comisión extra) sin capturar más ganancia. Rechazado con el mismo estándar que la idea #1 (basket multi-símbolo): probado a fondo, con tests, mantenido en el código apagado por defecto (`config/config.yaml` `exits.enabled: false`) por si una futura estrategia que sostenga menos posiciones más grandes lo vuelve relevante.
